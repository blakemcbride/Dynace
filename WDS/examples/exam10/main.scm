...
(define (display-values wind dlg)
  ...
  (vPrintf wind "Age = ~a~n" (gCtlShortValue dlg FIELD_AGE)))
...
(define (init-controls dlg)
  (let ((ctl nil))
    ...
    (set! ctl (gAddControl dlg NumericControl FIELD_AGE))
    (gNumericRange ctl 1.0 99.0 0)

    (set! ctl (gAddControl dlg DateControl FIELD_BIRTH_DATE))
    (gDateRange ctl 19000101 19991231 0)

    (set! ctl (gAddControl dlg NumericControl FIELD_MONEY))
    (gNumericRange ctl 0.0 100000.0 2)

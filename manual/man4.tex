...
Any classes whose instances allocate memory may need to override this method.
...
It returns the total number of bytes used by the object's instance variables.
...
It is also used by @code{Print::Object} (a method useful during the
...
abstract class to ensure that its subclasses implement the correct
methods.
...
@end example
@sp 1
See also:  @code{RespondsTo, InvalidObject::Method}
@end deffn
...
class of objects.  It doesn't affect any of its superclasses or subclasses and
has no effect if garbage collection is not being used.  It returns
@code{cls}.
@example
@group
@exdent Example:
...
This method returns the method object found.  If no method is found
...
@end group
@end example
This method is used to obtain the total size a direct instance of class
...
by @code{Print::Object} (a method useful during the debugging phase
...
All functionality of the Dynace class is accessed through its class methods.
...
allocated from the heap.  If an object is only being referenced by
...
parameter.  When using @code{dpp} strategies other than 1 (see the @code{dpp}
section) under certain circumstances @code{dpp} will alter the argument

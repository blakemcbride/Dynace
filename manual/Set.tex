...
that there are storage requirements associated with this number and
...
It will just be less efficient if many more elements than @code{size}
are added.
...
set is disposed of via @code{Dispose}.
...
however, that there are storage requirements associated with this number
...
(@code{s}).  The @code{Set} represented by @code{i} is not affected by
...
to print or display a representation of an object.  It is also used
by @code{Print::Object} (a method useful during the debugging phase of a
project) in order to directly print an object to a stream.

...
usage of a linked list quite a bit it comes at the expense of
...
return @code{NULL}.  The list, @code{i}, is not affected by this operation.
@example
@group
@exdent Example:
...
It does this without modifying the list.  Each argument must be a pointer to a
...
remaining list elements are ignored.
...
(@code{s}).  The link list represented by @code{i} is not affected by

...
bi-directional communication is needed two pipes may be used.
...
buffered before a thread has to be held.  If set too small the system will
...
threads may obtain the pipe object by the use of the @code{FindStr::Pipe}
method.  
@c @example
@c @group
@c @exdent Example:
...
operation with as many bytes as are available at the time.  There
will be no waiting.

The @code{rblock} flag affects read requests and the @code{wblock}
affects write requests.

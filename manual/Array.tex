...
by @code{s}.  The return type will be a pointer to the type of array it
is.  For example a @code{ShortArray} will return a pointer to a @code{short}
...
same shape and same values then @code{Equal} returns a 1.  @code{Equal}
returns 0 otherwise.
@c @example
@c @group
@c @exdent Example:
...
each consecutive dimension.  Note that the number of arguments after
